...
async def get_az_cli_logged_in_info(print_info: bool = True) -> AzCliLoggedInInfo:
    ...
    try:
        ...
        if print_info:
            ...
            print(f"Subscription: {colored(info.subscription_name, Colors.CYAN)} ({colored(info.subscription_id, Colors.CYAN)})")
        
        return info
    except subprocess.CalledProcessError:
        raise Exception("User not logged in to Azure CLI. Run 'az login'.")
    except FileNotFoundError:
        raise Exception("Azure CLI is not installed. Install it and run 'az login' before running this tool.")
...
class AzPIMClient:
    ...
    async def elevate(self, options: Dict):
        ...
        print(f"Elevating {colored(principal_id, Colors.CYAN)} to {colored(options['roleName'], Colors.CYAN)} for {colored(options['expirationDuration'], Colors.CYAN)}")
        ...
        try:
            ...
            print(result)
            print(colored(f"ELEVATION SUCCESSFUL for role {options['roleName']}", Colors.GREEN))
        except Exception as e:
            print(e)
            print(colored(f"Unable to elevate for role {options['roleName']}.", Colors.RED))
    
    async def get_role_definition_id(self, client: AuthorizationManagementClient, role_name: str, scope: str) -> str:
        """Get the role definition ID for a given role name."""
        role = None
        
        filter_str = "asTarget()"
        
        for item in client.role_eligibility_schedules.list_for_scope(scope=scope, filter=filter_str):
            if hasattr(item, 'expanded_properties') and \
               hasattr(item.expanded_properties, 'role_definition') and \
               hasattr(item.expanded_properties.role_definition, 'display_name') and \
               item.expanded_properties.role_definition.display_name == role_name:
                role = item
                break
        
        if role:
            role_id = role.expanded_properties.role_definition.id
            print(f"Found Role Definition {colored(role_name, Colors.CYAN)} with id {colored(role_id, Colors.CYAN)}")
            return role_id
        else:
            print(colored(
    ...
    async def get_principal_id(self, continue_on_failure: bool = False) -> str:
        ...
        try:
            ...
            if not continue_on_failure:
                print(colored("ERROR: Unable to get principal id of the current user.", Colors.RED))
                sys.exit(12)
            else:
...
class AzCliKeyVaultClient:
    ...
    async def create():
        ...
        try:
            ...
            print(colored("ERROR: User not logged in to Azure CLI. Run 'az login'.", Colors.RED))
            sys.exit(1)
        except FileNotFoundError:
            print(colored(
    ...
    def read_secret(self, vault_name: str, secret_name: str) -> Dict:
        ...
        result = subprocess.run(
            [AZ_COMMAND, "keyvault", "secret", "show", "--vault-name", vault_name, "--name", secret_name],
            capture_output=True,
            text=True,
            check=True
        )
    ...
    def write_secret(self, vault_name: str, secret_name: str, secret_value: str) -> Dict:
        """Write a secret to a vault."""
        result = subprocess.run(
            [AZ_COMMAND, "keyvault", "secret", "set", "--vault-name", vault_name, 
...
async def get_secret_list_with_elevation(client: AzCliKeyVaultClient, vault_name: str) -> List[Dict]:
...
async def get_secrets(client: AzCliKeyVaultClient, vault_name: str, shared: bool) -> List[Tuple[str, str]]:
    """Get all enabled secrets from a vault."""
    print(f"Getting existing {'shared' if shared else 'private'} secrets from {colored(vault_name, Colors.CYAN)} key vault.")
...
def read_dotenv() -> List[Tuple[str, str]]:
    ...
    for line in lines:
        ...
        if '=' in line:
            ...
            if '-' in key:
                raise ValueError(f"Invalid dotenv key '{key}' for key vault. Keys cannot contain dashes.")
...
async def push_secret(
    ...
    if secret_key in secrets:
        ...
        answer = input(f"  Are you sure you want to overwrite the value of {secret_key}? (y/n): ")
        if answer.lower() != 'y':
            print("Skipping...")
            return -1
...
def get_vault_names(dotenv: Dict[str, str]) -> Dict[str, Optional[str]]:
    ...
    return {
        'shared': param_shared_vault or dotenv.get('TYPEAGENT_SHAREDVAULT') or config['vault']['shared'],
...
async def push_secrets():
    ...
    shared_secrets = dict(await get_secrets(client, vault_names['shared'], True))
    private_secrets = dict(await get_secrets(client, vault_names['private'], False)) if vault_names['private'] else {}
...
async def pull_secrets():
    ...
    shared_updated = await pull_secrets_from_vault(client, vault_names['shared'], True, dotenv)
    private_updated = None
    if vault_names['private']:
        private_updated = await pull_secrets_from_vault(client, vault_names['private'], False, dotenv)
    ...
    if dotenv.get('TYPEAGENT_SHAREDVAULT') != vault_names['shared']:
        print("  Updating TYPEAGENT_SHAREDVAULT")
        dotenv['TYPEAGENT_SHAREDVAULT'] = vault_names['shared']
        updated += 1
    
    if vault_names['private'] and dotenv.get('TYPEAGENT_PRIVATEVAULT') != vault_names['private']:
...
async def main():
    ...
    parser.add_argument('command', nargs='?', choices=['push', 'pull', 'help'], 
                       default='pull', help='Command to execute')
