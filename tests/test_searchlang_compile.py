...
class TestDateRangeFromDatetimeRange:
    ...
    def test_date_only_stop_covers_whole_day(self) -> None:
        # Regression test for the bug reported in PR #296: a stop date with no
        # time used to compile to that day's midnight, so the whole final day
    ...
    def test_stop_with_time_is_exclusive_bound(self) -> None:
        ...
        assert (
            datetime.datetime(2023, 1, 5, 9, 59, 59, tzinfo=datetime.timezone.utc) in dr
        )
        assert (
            datetime.datetime(2023, 1, 5, 10, 0, tzinfo=datetime.timezone.utc) not in dr
        )
